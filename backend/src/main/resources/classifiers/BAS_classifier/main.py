...
flags.DEFINE_boolean('save', False, 'Save input for future training')
...
def test_method():
    ...
    with open(f"{os.path.dirname(__file__)}\\{FLAGS.classifier}_model.pkl", 'rb') as file:
        model = pickle.load(file)
    prediction = model.predict(df)
    if FLAGS.save:
        main_csv = pd.read_csv(os.path.dirname(__file__) + "\\BAS.csv")
        df['Outcome'] = prediction
        df['Gender'] = df['Gender'].map(reversed_gender_dictionary)
        main_csv = main_csv.append(df, ignore_index=True)
        main_csv.to_csv(os.path.dirname(__file__) + "\\BAS.csv", index=False)
    print(prediction)


def train_method():
    df = pd.read_csv(os.path.dirname(__file__) + "\\BAS.csv")
    df['Gender'] = df['Gender'].map(gender_dictionary)
    y = df['Outcome']
    ...
    classifier.fit(x, y)
    with open(f"{os.path.dirname(__file__)}\\{FLAGS.classifier}_model.pkl", 'wb') as file:
